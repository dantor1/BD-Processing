...
object JdbcProvisioner {
  ...
  def main(args: Array[String]) {
    ...
    try {
      ...
      statement.execute("CREATE TABLE IF NOT EXISTS bytes(timestamp TIMESTAMP, id TEXT, value INT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS bytes_hourly(timestamp TIMESTAMP, id TEXT, value INT, type TEXT)")
      statement.execute("CREATE TABLE IF NOT EXISTS user_quota_limit(email TEXT, usage BIGINT, quota INT, timestamp TIMESTAMP)")
      statement.execute("CREATE TABLE IF NOT EXISTS user_metadata(id TEXT, name TEXT, email TEXT, quota INT)")
